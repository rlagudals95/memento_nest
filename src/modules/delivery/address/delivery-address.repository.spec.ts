...
import { CreateDeliveryAddress } from './models/delivery-address.dto';
import { DeliveryAddressRepository } from './delivery-address.repository';

describe.skip('DeliveryAddressRepository', () => {
  let environment: StartedDockerComposeEnvironment;
  let repository: DeliveryAddressRepository;

  beforeEach(async () => {
    environment = await startExternalDependencies();
    initConnectionForLocalDynamoDB();
    repository = await createProvider(DeliveryAddressRepository);
  ...
  it('test create', async () => {
    const item: CreateDeliveryAddress = {
      userId: 'testuser1',
  ...
  it('test findByUserId', async () => {
    const items: CreateDeliveryAddress[] = [
      {
        userId: 'testuser1',
        addressId: 'testaddress1',
        displayName: '기본배송지',
